...
const Trees = () => {
  ...
  const [treeAvailable, setTreeAvailable] = useState<number>();
  ...
  const fetchTree = async () => {
    ...
    const temp = res.filter((t:TreeRes) => t.assignments.length === 0);
    setTreeAvailable(temp.length);
    const t = res.filter((t : TreeRes) => t.assignments.length > 0);
    setTreeTask(t.length);
    return res;
  }
  ...
  const filteredData = trees.filter((row) => {
    ...
    const matchesFilterType =
      ...
      (filterType === "haveTask" && row.assignments.length > 0);
